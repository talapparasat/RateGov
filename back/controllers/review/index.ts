    ...
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            if(newRate >=0 && newRate <= 5) {
                serviceProvider.rate = newRate;
            }

            serviceProvider.reviewCount = serviceProvider.reviewCount + 1;

            await serviceProvider.validate()
...
const remove = async (reviewId: string) => {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            let newRate;
            if(!serviceProvider.rate) {
                newRate = 0;
            } else if(serviceProvider.rate == 0) {
                newRate = 0;
            } else {
                newRate = ((serviceProvider.rate * serviceProvider.reviewCount) - review.rate) / (serviceProvider.reviewCount - 1);
            }
            ...
            if(serviceProvider.reviewCount > 0) {
                serviceProvider.reviewCount = serviceProvider.reviewCount - 1
            }
