...
class Analytics extends Component{
    ...
    onChange = e => {
        ...
        const filterTitle = (start && end) ? `${e[0]._d.getDate()} ${monthNames[e[0]._d.getMonth()]}  ${e[0]._d.getFullYear()} - ${e[1]._d.getDate()} ${monthNames[e[1]._d.getMonth()]}  ${e[1]._d.getFullYear()}` : ''
        this.setState({
    ...
    exportAnalytics = () => {
        ...
        this.props.getAnalyticsExport(this.handleErrorReview, this.handleLoadingReview, data);
    }
    render() {
        ...
        const contentFilter = (
            <Menu onSelect={this.onSelect}>
                ...
                <div style={{display: "flex", justifyContent: "center", margin: '10px 0'}}>
                    <button onClick={this.handleOkFilter} style={{padding: `20px 20px`, outline: `none`, cursor: 'pointer'}} className="excel-button">Применить фильтр</button>
                </div>
            </Menu>
        )
        const contentRegion = (
            <Tabs className="analytics-tabs">
                <TabPane tab="Регионы" key="1">
                    <Menu onSelect={this.onSelectRegion} style={{overflow: 'auto', height: '400px'}}>
                        <Menu.Item key={'1'}>
                            Все регионы
                        </Menu.Item>
                        {regions}
                ...
                <TabPane tab="Районы" key="2">
                    <Menu onSelect={this.onSelectRaion} style={{overflow: 'auto', height: '400px'}}>
                        <Menu.Item key={'2'}>
                            Все районы
                        </Menu.Item>
                        {raions}
                    ...
                    <div className="analytics-inner">
                        <div className="analytics-top">
                            <div className="filter-region">
                                <div>
                                    <h2>Аналитика / </h2>
                                    <Popover onVisibleChange={this.handleVisibleChangeRegion} visible={this.state.visibleRegion} placement="bottom" title={textRegion} content={contentRegion} trigger="click">
                                        <h2 style={{cursor: 'pointer'}}>{this.state.textTitle} <Icon style={{fontSize: '12px'}} type="caret-down"/></h2>
                                    </Popover>
                                </div>
                                <p>по Механизму обратной связи и разрешения проблем</p>
                            </div>
                            {/* <div className="filter-analytics">
                                <Popover onVisibleChange={this.handleVisibleChange} visible={this.state.visible}  placement="bottom" title={textFilter} content={contentFilter} trigger="click">
                                    <button><Icon style={{marginRight:'7px'}} type="filter"/>{this.state.filterTitle}</button>
                                </Popover>
                                <button className="excel-button" onClick={this.exportAnalytics}><Icon style={{marginRight:'7px'}} type="file"/>Выгрузить в Excel</button>
                            </div> */}
                        </div>
                        {/* <ReviewBlock review={analytics ? analytics.reviewsCount : {}}/> */}
                        {/* <div className="charts">
                            <LineChart categories={analytics ? analytics.categoriesRating : {}} region={this.state.keyRegion}  filter={this.state.period} start={this.state.start ? this.state.start._d : ''} end={this.state.end ? this.state.end._d : ''}/>
                            <div className="bar-chart">
                                <BarChart social={analytics ? analytics.social : {}} region={this.state.keyRegion}  visible={!this.state.visible}  filter={this.state.period}/>
                            </div>
                        </div> */}
                        {/* {(this.state.keyRegion === '1') && analytics ? <div>
                            <PeopleBlock visible={!this.state.visible} filter={this.state.key}/>
                            <Rating raions={analytics ? analytics.raionsRating :  {}}  regions={analytics ? analytics.regionsRating : {}} services={analytics ? analytics.servicesRating : {}}/></div> : ''} */}
                    </div>
                </div>
            </Spin>
            </div>
...
export default connect(mapStateToProps, {getAnalytics, getNavigation, getAnalyticsExport}) (Analytics);
