...
import {getUserById, saveUser} from "../../actions/userActions";
...
class AddSupervisor extends Component{
    ...
    handleOk=()=>{
        console.log(this.state.mobileCode, this.state.mobilePhone)
        let data={
            ...
            path:'/dashboard/supervisor/connect'
        }
        if(this.props.user.user[0] && (this.state.img === this.props.user.user[0].image)){
            data.image = null
        }
        else{
            data.image=this.state.img
        }
        if(this.props.user.profile.role === 'superadmin' || localStorage.getItem('role') === 'superadmin'){
            data.path = '/dashboard/supervisor/connect'
        }
        else if(this.props.user.profile.role ===' admin' || localStorage.getItem('role') === 'admin'){
            data.path = '/dashboard/supervisor/confirm'
        }
                ...
                <Spin spinning={this.state.loading}>
                    ...
                    <div className="container-add">
                        ...
                        <div className="add-inner">
                            <h2 className="h5-title">Шаг 1/2. Добавить супервайзера</h2>
                            <p className="title-text">Заполните данные нового Супервайзера.</p>
                            <Form  onSubmit={this.handleSubmit}  style={{width:"100%"}}  layout={"vertical"}>
                                <Form.Item label="Данные супервазйера">
                                    <Upload
                                        name="img"
...
export default connect(mapStateToProps,{saveUser,getUserById}) (withRouter(AddSupervisor))
